<template>
  <div class="h-full flex flex-col p-4 overflow-auto bg-gradient-to-b from-[#151515] to-[#101010] dark:from-[#0d0d0d] dark:to-[#080808]">
    <!-- 平台和模型选择栏 -->
    <div class="flex items-center gap-4 mb-4 p-3 bg-[#1a1a1a] dark:bg-[#121212] border border-[#2a2a2a] dark:border-[#333333]">
    ...
    <div class="grid grid-cols-2 gap-4 flex-1 overflow-hidden">
      ...
      <div class="flex flex-col gap-4 overflow-hidden">
        <!-- System Prompt -->
        <div class="flex-1 overflow-hidden border border-[#2a2a2a] dark:border-[#333333] bg-[#1a1a1a] dark:bg-[#121212] shadow-lg rounded-lg">
          <div class="p-3 h-full flex flex-col">
            <div class="flex justify-between items-center mb-2">
              ...
              <div class="flex gap-2">
                ...
                >
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 mr-1" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <div class="flex flex-wrap gap-1 mb-2">
              <span 
                v-if="currentPromptVersion?.requirements && currentPromptVersion.requirements !== '初始版本'" 
            ...
            <div class="flex-1 relative overflow-hidden">
              <textarea
                ...
                placeholder="输入系统提示词..."
                class="w-full h-full resize-none bg-[#252525] border-[#333333] text-purple-200 font-mono p-2 rounded-md focus:ring-purple-500/50 focus:border-purple-500/50"
              ></textarea>
            </div>
          </div>
        </div>
            ...
            <div class="space-y-2 max-h-[300px] overflow-y-auto">
              ...
              >
                ...
                <textarea
                  ...
                  @input="updateTestCase(testCase.id, $event.target.value)"
                ></textarea>
              </div>
            </div>
          </div>
      ...
      <!-- 右侧内容 - 测试结果 -->
      <div class="border border-[#2a2a2a] dark:border-[#333333] bg-[#1a1a1a] dark:bg-[#121212] shadow-lg rounded-lg overflow-hidden flex flex-col">
        <div class="p-3 flex-1 overflow-hidden flex flex-col">
          <div class="flex justify-between items-center mb-2">
            <h3 class="text-sm text-green-400">测试结果</h3>
            <button 
              @click="startOptimization" 
              :disabled="isOptimizing || testCases.some(tc => !tc.question)"
          ...
          <div class="flex-1 overflow-auto">
            <div v-if="isOptimizing" class="flex items-center justify-center h-full">
              <div class="text-center">
                <svg class="animate-spin h-8 w-8 mx-auto mb-4 text-green-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                  <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                  <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                </svg>
            ...
            <div v-else-if="filteredResponses.length === 0" class="flex items-center justify-center h-full">
              <div class="text-center text-gray-500">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 mx-auto mb-4 text-gray-600" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              ...
              >
                <div class="flex justify-between items-start mb-2">
                  ...
                  <div class="flex items-center gap-1">
                    <button 
                      @click="setResponseFeedback(response.id, 'thumbsUp', !response.thumbsUp)" 
                      :class="[
                    ...
                    <button 
                      @click="setResponseFeedback(response.id, 'thumbsDown', !response.thumbsDown)" 
                      :class="[
                        'h-6 w-6 rounded flex items-center justify-center', 
                ...
                <div v-if="response.thumbsDown" class="mt-2">
                  <textarea
                    ...
                    @input="setResponseFeedback(response.id, 'feedback', $event.target.value)"
                  ></textarea>
                </div>
              </div>
            </div>
